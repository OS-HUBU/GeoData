    ...
    let option = {
      ...
      formatter: function (params) {
        if(params.componentSubType ==='bar'){
          ...
          return `${params.data[1]}</br> developer ：${params.data[0].toLocaleString()}`
        }
        else{
          return params.name
        }
      ...
      title:[
        {
          text: '{larger|' + startYear + '}年\n'  + 'Top15  regions with most developers in ' + rankTitle,
         ...
         function updateDataAndShowTip(){
          var option =myChart.getOption();//获得option对象
                 ...
                 let source = transformedData.slice(1).filter(function (d) {
                   ...
                   option.title[0].text = '{larger|' + year + '}\n'+ 'Top15  regions with most developers in ' + rankTitle;
                 myChart.setOption(option)
            // dataIndex ++ ;
         }
